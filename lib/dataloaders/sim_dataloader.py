...
class SimDataloader(Dataset):
    ...
    def __init__(self, configs, mode='train', set_size='', filtering=False, subset=False, subset_nb=0,
                 remove_duplicated='/data/resist_data/adaIn_maps/sim_crack/train/*'):
        ...
        if filtering:
            ...
            if not self.image_arr is None:
                ...
                already_done = [os.path.join(os.path.dirname(self.image_arr[0]),
                                             os.path.basename(pp).replace('gt', 'render').replace('.npy', '')) for pp in
                                already_done]
    ...
    def transform_crop_downsize_combo(self, image, segmask, depth=None, normal=None, pmi_gt=None, adain_img=None):
        '''
        ...
        while (np.count_nonzero(mask_cropped[:, :, 0]) < 200) and (
            ...
            if self.configs.arch_name != 'pmiunet':
                ...
                if not adain_img is None:
                    adain_img_cropped = adain_img[i:i + h, j:j + w, :]
            else:
        ...
        if not adain_img is None:
            adain_img = adain_img_cropped
            adain_img = torch.tensor(adain_img).float()
        if self.configs.input_ch == 1 and self.configs.arch_name != 'pmiunet':
            ...
            if not adain_img is None:
                input_image = np.dot(adain_img[..., :3], [0.299, 0.587, 0.114])
                input_image = np.expand_dims(input_image, axis=0)
                input_torch = torch.tensor(input_image).float()
            if not pmi_gt is None:
                pmi_gt = pmi_gt[:, :, 0]
    ...
    def __getitem__(self, index):
        ...
        if self.configs.arch_name == 'pmiunet':
            ...
            if self.configs.adain:
                # use images augmented  with Adaptive Instance Normalization if they are available
                ...
                if not use_orginal:
                    ...
                    if os.path.isdir(adain_path):
                        # print('exists!!')
                        if not os.listdir(adain_path):  # TODO correct thois
                            pass
                        else:
                            adain_path = random.choice(glob.glob(adain_path + '/*'))
                            adain_img = np.load(adain_path)[0]
                            adain_img = adain_img.transpose(1,2,0)
        ...
        if self.configs.arch_name == 'munet':
            ...
            if self.configs.resize_crop_input:
                img, gt, pmi_gt = self.transform_crop_downsize_combo(image, mask, pmi_gt=pmi_gt, adain_img=adain_img)
            else:
